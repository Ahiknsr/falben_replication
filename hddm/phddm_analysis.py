...
def fit_model(data, model_name, depends_on, exp_label, rep_label):
    ...
    m.sample(
        ...
        dbname=db_name,
        db="pickle",
        progress_bar=True,  # Disabled progress bar for clean parallel logs
    )
    m.save(os.path.join(
...
def process_dataset(exp_num: int, rep_label: str):
    ...
    # STEP 1: Preprocess
    print(f"\n  [Step 1 / 6] Preprocessing {exp_label} {rep_label}…")
    ...
    print(f"\n  [Step 2 / 6] Fitting HDDM models "
          f"({len(EXP_MODELS[exp_num])} models) for {exp_label} {rep_label}…")
    models_dict = {}
    ...
    # STEP 3: DIC comparison
    print(f"\n  [Step 3 / 6] DIC comparison for {exp_label} {rep_label}…")
    ...
    print(f"\n  [Step 4 / 6] Extracting parameters for {exp_label} {rep_label}…")
    extract_parameters(
        best_model, exp_label, rep_label, best_name
    )

    # STEP 5: Z-bias tests
    print(f"\n  [Step 5 / 6] Testing z bias for {exp_label} {rep_label}…")
    zbias = test_z_bias(
        best_model, exp_num,
    ...
    # STEP 6: PPC
    print(f"\n  [Step 6 / 6] Posterior Predictive Check for {exp_label} {rep_label}…")
...
def main():
    ...
    print(f"Initiating parallel processing of {len(datasets)} datasets...")
    
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = [executor.submit(process_dataset, exp_num, rep_label) for exp_num, rep_label in datasets]
